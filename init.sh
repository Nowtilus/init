...
apt-get update
apt-get -y install nfs-kernel-server wget
...
wget https://sourcestreamproxy.blob.core.windows.net/download/nginx_1.20.2-1~focal_amd64.deb
dpgk -i download/nginx_1.20.2-1~focal_amd64.deb
...
# chmod +x /usr/sbin/nginx
 ln -s /mnt/ramdisk /usr/share/nginx/html
